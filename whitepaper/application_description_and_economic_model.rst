...
The wesion Network uses distributed peer-to-peer network technology to build a distributed,
...
The wesion Network provides distributed services such as VPN, CDN, DNS, and DDoS defense.
The :ref:`wesion` with stable value scale will be used in the ecology and produce
an incentive effect. Everyone can participate in distributed network construction.
We believe the wesion Network,
...
**1 wesion = 100 GB client network traffic (price at IDC is $8.0 - 8.5)**

The user running the service node makes use of idle network resources to earn wesions,
and the user or organization that needs the service uses wesions exchange to settle
...
With the help of wesion Network,
users participating in the ecology can easily run wesion node programs on computers,
...
The longer the time is online, the more wesions can be obtained,
...
The contract is settled automatically, the node inside the wall pays the :ref:`wesion` as the demand side,
and the outside node uses the idle bandwidth as the service party to provide
the encrypted trafficproxy service and automatically accept the :ref:`wesion` settlement,
...
mobile devices, or even just a RaspberryPI device running wesion node programs,
...
but writes the ratio of :ref:`wesion` to network traffic flow resource service
as a constant definition in the underlying code.


.. _wesion:

wesion -- Unique token in the wesion Network ecosystem
-------------------------------------------------------
...
:ref:`wesion` will be strictly based on the ERC20 token standard,
...
we defined per unit of ``wesion`` as the client traffic of 100GBytes in the wesion Network,
...
The wesion Network pioneered the definition of the **Proof of Traffic** consensus mechanism,
the wesion node has only a very low performance requirement for the device,
...
but also wesion’s dollar price is more stable,
...
Therefore, 100 GB client traffic flow, anchored to 1 wesion,
...
The wesion dollar price should increase to the internal value gradually and be relatively stable
-----------------------------------------------------------------------------------------------
...
At the same time, we expect that the wesion value scale
after ecological maturity is relatively stable
(that is, the legal currency price is relatively stable),
and the basic setting of 1 wesion = 100 GB can ensure that
the market circulation price of wesion gradually matures
